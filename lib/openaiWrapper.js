...
export async function queryOpenAI(query, options = {}) {
  const defaultModel = LLM_PROVIDER === 'ollama' ? OLLAMA_MODEL : (process.env.OPENAI_DEFAULT_MODEL || 'gpt-4o-mini');
  console.log('[queryOpenAI] Default model calculated:', defaultModel, 'LLM_PROVIDER:', LLM_PROVIDER);
