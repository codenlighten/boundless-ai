...
export class TerminalChatbot extends MemoryChatbot {
  constructor(sessionPath = './session.json', options = {}) {
    ...
    this.disableWhitelist = options.disableWhitelist || false;
    
    // Command whitelist - only these commands are allowed (unless disabled)
    this.allowedCommands = options.allowedCommands || [
      ...
      'sudo', 'apt', 'apt-get', 'systemctl', 'service', 'pm2', // System administration
      'ufw', 'iptables', 'useradd', 'usermod', 'passwd' // Security/user management
    ];
    
    // Commands that require confirmation (still enforced even when whitelist disabled)
    this.dangerousCommands = new Set([
      'rm', 'rmdir', 'kill', 'killall', 'pkill', 'chmod', 'chown',
      'sudo', 'apt', 'apt-get', 'systemctl', 'service', // System operations
      'ssh', 'scp', // SSH operations
      'ufw', 'iptables', 'useradd', 'usermod', 'userdel', 'passwd', // Security critical
  ...
  isCommandAllowed(command) {
    // If whitelist is disabled, allow all commands (dangerous commands still need approval)
